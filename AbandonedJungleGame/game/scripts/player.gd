...
func _physics_process(delta: float) -> void:
	...
	if dash_timer > 0.0:
		...
		return
	if not is_on_floor(): velocity.y = minf(velocity.y + gravity * delta, max_fall_speed)
	...
	if jump_buffer_timer > 0.0 and coyote_timer > 0.0: _perform_jump()
	if Input.is_action_just_released("jump") and velocity.y < -120.0: velocity.y *= jump_release_multiplier
	if Input.is_action_just_pressed("dash") and dash_cooldown_timer <= 0.0:
		dash_timer = dash_duration
		dash_cooldown_timer = dash_cooldown
	if Input.is_action_just_pressed("attack") and attack_timer <= 0.0: _start_attack()
	move_and_slide()
	if is_on_floor(): coyote_timer = coyote_time
	_update_visual_state()
...
func _update_visual_state() -> void:
	if dead: return
	if hurt_timer > 0.0: _play_if_needed(&"hurt")
	elif attack_timer > 0.0: _play_if_needed(&"attack")
	elif not is_on_floor(): _play_if_needed(&"jump")
	elif absf(velocity.x) > 20.0: _play_if_needed(&"walk")
	else: _play_if_needed(&"idle")

func _play_if_needed(name: StringName) -> void:
	if sprite.sprite_frames.has_animation(name) and sprite.animation != name: sprite.play(name)
	if animation_player.has_animation(name) and animation_player.current_animation != name: animation_player.play(name)
...
func _start_attack() -> void:
	...
	for body in attack_area.get_overlapping_bodies():
		if body != self and body.has_method("take_damage"): body.take_damage(attack_damage, global_position)

func take_damage(amount: int, from_position: Vector2) -> void:
	if dead or hurt_timer > 0.0: return
	hp = maxi(hp - amount, 0)
	hurt_timer = hurt_invulnerability
	hurt_sfx.play()
	if from_position != Vector2.ZERO:
		var away := signf(global_position.x - from_position.x)
		if away == 0.0: away = -facing
		velocity = Vector2(away * 310.0, -260.0)
	health_changed.emit(hp, max_hp)
	if hp <= 0: _die()
	else: _play_if_needed(&"hurt")
